...
public class UClothLaserGrabber3 : MonoBehaviour
{
    ...
    [Tooltip("Kéo object chứa script ClothGrabLogger vào đây. Nếu để trống, hệ thống sẽ không log dữ liệu.")]
    public ClothGrabLogger dataLogger;
    ...
    private void ProcessMainHand()
    {
        ...
        else if (!triggerPressed && wasGrabbingMain)
        {
            // BÁO CÁO NHẢ VẢI CHO LOGGER
            if (dataLogger != null && isGrabbingMain)
            {
                dataLogger.NotifyClothDropped(cachedNodePosMain);
            }
    ...
    private async void FindNodeAsyncMain(Vector3 hitPoint, Transform controllerAtTime)
    {
        ...
        if (closestPoints != null && closestPoints.Count > 0)
        {
            ...
            cachedNodePosMain = hitPoint; 
            isGrabbingMain = true;

            // BÁO CÁO BẮT ĐẦU BÁM VẢI CHO LOGGER
            if (dataLogger != null)
            {
                dataLogger.NotifyGrabStarted();
            }
        }
        isFindingNodeMain = false;
    }
    ...
    void OnDestroy()
    {
        ...
        if (triggerAction != null && triggerAction.action != null) triggerAction.action.Disable();
